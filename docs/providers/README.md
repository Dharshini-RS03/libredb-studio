...
| RisingWave | `postgres` | Partial | RisingWave 3.0.4 | **Re-probed after #1075: the object browser now lists tables, columns, and materialized views.** RisingWave does not provide the PostgreSQL `json` type or `json_agg()` / `json_build_object()`, but it provides the `jsonb` equivalents. The provider falls back to `jsonb_agg()`, `jsonb_build_object()` and `jsonb` casts, so column and type metadata can be read. Row counts and sizes remain blank because RisingWave does not provide `pg_class.reltuples` or `pg_total_relation_size()`. The monitoring dashboard still loads with every statistic marked unavailable because RisingWave has no pg statistics catalog; slow-query and active-session panels remain empty because parameterised `LIMIT` is rejected. |
