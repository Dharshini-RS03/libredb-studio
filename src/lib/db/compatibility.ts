...
export const WIRE_COMPATIBLE_ENGINES: readonly WireCompatibleEngine[] = [
  ...
  },
  {
    ...
    caveats: [
      ...
      "The object browser lists tables and materialized views, and column details are available through the provider's json-to-jsonb fallback: RisingWave rejects PostgreSQL json expressions such as CAST(NULL AS json), json_agg() and json_build_object(), but accepts their jsonb equivalents. The provider retries the affected describeObject() and describeObjects() queries with those jsonb forms.",
    ],
  },
  {
    name: "OrioleDB",
